...
buttons = pd.read_excel("buttons.xlsx")
contigency = pd.read_excel("contigency.xlsx")
...
#exp = data.ExperimentHandler(name='DROIntegrity', dataFileName=filename + 'PointsData')
...
BreakTxt = visual.TextStim(win=win, name='BreakTxt',
    text='hey you can take a break now! You have ' + str(Duration - PhaseTimer.getTime()) + ' seconds left in your break, \n however you can click the button to end it early \n',
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
    languageStyle='LTR',
...
for thisPhaseSelector in PhaseSelector:
    ...
    while continueRoutine:
        ...
        if BreakButton.status == NOT_STARTED and Phase == "Break" and PhaseTimer.getTime() >= 4:
            # keep track of start time/frame for later
